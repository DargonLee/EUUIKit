...
typedef NS_ENUM(NSUInteger, EUFeatureItemLocation) {
    EUFeatureItemLocationDefault = 0,
    EUFeatureItemLocationUp = 1 << 1,
    EUFeatureItemLocationLeft = 1 << 2,
    EUFeatureItemLocationDown = 1 << 3,
    EUFeatureItemLocationRight = 1 << 4
};


@implementation UIView (EUFeatureGuideView)
...
- (void)layoutSubviewsWithFeatureItems:(NSArray<EUFeatureItem *> *)featureItems
{
    ...
    [featureItems enumerateObjectsUsingBlock:^(EUFeatureItem * featureItem, NSUInteger idx, BOOL * _Nonnull stop) {
...
/**
 *  计算EUFeatureItem所处的位置
 *
 *  @param featureItem featureItem
 *
 *  @return return EAFeatureItemLocation
 */
- (EUFeatureItemLocation)getLocationForFeatureItem:(EUFeatureItem *)featureItem
{
    
    EUFeatureItemLocation location = EUFeatureItemLocationDefault;
    ...
    if(frame.size.width <= squareWidth * (split - 1))
    {
        ...
        {
            location |= EUFeatureItemLocationRight;
        }
        //右边
        else if((rightSpace - leftSpace) >= squareWidth)
        {
            location |= EUFeatureItemLocationLeft;
        }
    }
    ...
    {
        location |= EUFeatureItemLocationDown;
    }
    //下边
    else if((bottomSpace - topSpace) > squareHeight)
    {
        location |= EUFeatureItemLocationUp;
    }
    else if(featureItem.introduceAlignmentPriority == EUFeatureItemAlignmentBottomFirst)
    {
        location |= EUFeatureItemLocationUp;
    }
    else
...
- (void)layoutWithFeatureItem:(EUFeatureItem *)featureItem
{
    ...
    EUFeatureItemLocation location = [self getLocationForFeatureItem:featureItem];
    
    CGRect introduceFrame = introduceView.frame;
    const CGFloat verticalSpacing = 10;
    
    //箭头方向向上
    if(location & EUFeatureItemLocationUp || location == EUFeatureItemLocationDefault)
    {
        //将箭头的锚点移动到顶部中间
        ...
        //箭头方向左上
        if(location & EUFeatureItemLocationLeft)
        {
        ...
        //箭头方向右上
        else if(location & EUFeatureItemLocationRight)
        {
            CGAffineTransform transform = indicatorImageView.transform;
            indicatorImageView.transform = CGAffineTransformRotate(transform,M_PI / 4);
    ...
    else if(location & EUFeatureItemLocationDown)
    {
        //是否需要布局按钮
