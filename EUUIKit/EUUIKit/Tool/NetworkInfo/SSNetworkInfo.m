...
+ (nullable NSString *)wiFiBroadcastAddress {
    ...
    @try {
        ...
        // Make a string for the address
        NSString *BroadcastAddress = [NSString stringWithFormat:@"%lu.%lu.%lu.%lu",(unsigned long) ((BA & 0xFF000000) >> 24),(unsigned long)
                                      ((BA & 0x00FF0000) >> 16),(unsigned long) ((BA & 0x0000FF00) >> 8),(unsigned long) (BA & 0x000000FF)];
